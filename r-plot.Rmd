...
```{r fig.dim=c(6,4), out.extra='style="width:80%;display:block;margin:auto"'}
c(
...
seq_len_50 <- function(x) {seq(min(x), max(x), length.out = 50)}
...
as_ggplot_list(
    ...
    hist(obama_vs_mccain$Obama, seq_len_50, main = "A function for the bin edges")
...
```{r}
as_ggplot_list(
    plot(1:25, pch = 1:25, bg = "blue", main = 'plotting character, pch'),
    plot(bat[,c("H","R","SO","BB","HR")], main = 'pairs of correlation')
) %>% gridExtra::grid.arrange(grobs = ., nrow = 1, widths = c(1, 3))
```
...
`persp()` draws a plot of a three-dimensional surface for a specific perspective.
